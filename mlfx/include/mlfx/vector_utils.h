...
class VectorBase {
 ...
 protected:
  template <size_t CurrentIndex,
            ...
            typename OtherType,
            typename... Args>
  void initWithList(const Vector<OtherDimension, OtherType>& arg1,
                    const Args&... args);

  // Some old compilers consider this function an alternative for
  // initWithList(Vector) when the variant above is more precise. Use SFINAE on
  // the return value to hide this variant for non-arithmetic types. The return
  // value is still void.
  ...
  initWithList(OtherType arg1, const Args&... args);
...
std::ostream& operator<<(std::ostream& ostream,
                         const VectorBase<Dimension, Type>& vector);
...
template <size_t Dimension, typename Type>
template <typename Type2>
VectorBase<Dimension, Type>::VectorBase(
...
// However if we define the compound constructor for when it has a single
// arguments, then calling Vector2(0.0) will be ambiguous. To solve this we take
// advantage of there being a single compound constructor with a single
// argument, which is the copy constructor. We end up with three constructors:
//  - the scalar constructor
//  - the copy constructor
//  - the compound constructor for two or more arguments, hence the arg1, and
//  arg2 here.
template <size_t Dimension, typename Type>
template <typename Arg1, typename Arg2, typename... Args>
VectorBase<Dimension, Type>::VectorBase(const Arg1& arg1,
                                        const Arg2& arg2,
                                        const Args&... args) {
...
template <size_t CurrentIndex,
          ...
          typename OtherType,
          typename... Args>
void VectorBase<Dimension, Type>::initWithList(
...
VectorBase<Dimension, Type>::initWithList(OtherType arg1, const Args&... args) {
  static_assert(CurrentIndex + 1 <= Dimension,
                "Too much data in the vector constructor.");
...
void VectorBase<Dimension, Type>::initWithList() const {
  static_assert(CurrentIndex == Dimension,
                "Not enough data in the vector constructor.");
}
...
template <size_t Dimension, typename Type>
void VectorBase<Dimension, Type>::Store(const Vector<Dimension, Type>& source,
                                        Type* destination) {
  for (size_t i = 0; i < Dimension; ++i) {
...
template <size_t Dimension, typename Type>
bool VectorBase<Dimension, Type>::operator==(
...
template <size_t Dimension, typename Type>
Type VectorBase<Dimension, Type>::dot(
    const VectorBase<Dimension, Type>& other) const {
  Type sum = Type();
  for (size_t i = 0; i < Dimension; ++i) {
